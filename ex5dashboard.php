...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        $balAmt = $row["balance"];
        // echo '<script>alert("Balance of Account Number : ' . $_SESSION['accno'] .'\nRs. '. $row["balance"] . '")</script>';
    }
}
if (isset($_POST['submit'])) {
    ...
    if ($_POST['submit'] == 'Deposit') {
        ...
        $updateQuery = "update accounts SET balance=" . $newBal . " where accno = '" . $_SESSION['accno'] . "';";
    ...
    if ($_POST['submit'] == 'Withdraw') {
        ...
        if ($balAmt - $amt < 500) {
            echo '<script>alert("Dear Account Holder,\nYour need to maintain a minimum balance of Rs. 500 at all times")</script>';
        } else {
            $newBal = (float)$balAmt - (float)$amt;
            $updateQuery = "update accounts SET balance=" . $newBal . " where accno = '" . $_SESSION['accno'] . "';";
            if (mysqli_query($conn, $updateQuery)) {
    ...
    if ($_POST['submit'] == 'Create') {
        ...
        $insertQuery = "insert into accounts values(" . $new_accno . ",'" . $new_password . "','" . $new_name .
...
<body>
    ...
    <div class="center">
        ...
        <!-- <h1 id="balance" style="color:white;">Balance is Rs. <?php echo $balAmt; ?></h1> -->
